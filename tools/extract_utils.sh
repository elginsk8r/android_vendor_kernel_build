    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            if [ -a "$DUMPDIR"/payload.bin ]; then
                ...
                python "$LINEAGE_ROOT"/vendor/ev/build/tools/extract_android_ota_payload.py "$DUMPDIR"/payload.bin "$DUMPDIR" 2>&1
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$LINEAGE_ROOT"/vendor/ev/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
