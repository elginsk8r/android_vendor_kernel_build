...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ANDROID_ROOT"/vendor/ev/build/tools/smali/baksmali.jar
        export SMALIJAR="$ANDROID_ROOT"/vendor/ev/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ANDROID_ROOT"/vendor/ev/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ANDROID_ROOT"/vendor/ev/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            if [ -a "$DUMPDIR"/payload.bin ]; then
                ...
                python "$ANDROID_ROOT"/vendor/ev/build/tools/extract_android_ota_payload.py "$DUMPDIR"/payload.bin "$DUMPDIR" 2>&1
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$ANDROID_ROOT"/vendor/ev/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
