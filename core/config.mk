...
PREBUILT_TOOLS := prebuilts/build-tools
PREBUILT_KERNEL_TOOLS := prebuilts/evervolv-tools
...
KERNEL_MAKE_FLAGS += -j$(shell $(PREBUILT_KERNEL_TOOLS)/$(HOST_PREBUILT_TAG)/bin/nproc --all)
...
TOOLS_PATH_OVERRIDE := \
    LD_LIBRARY_PATH=$(BUILD_TOP)/$(PREBUILT_KERNEL_TOOLS)/$(HOST_PREBUILT_TAG)/lib:$$LD_LIBRARY_PATH \
    PERL5LIB=$(BUILD_TOP)/$(PREBUILT_KERNEL_TOOLS)/common/perl-base

ifeq ($(KERNEL_ARCH),arm64)
  # Add 32-bit GCC to PATH so that arm-linux-androidkernel-as is available for CONFIG_COMPAT_VDSO
  TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/$(PREBUILT_KERNEL_TOOLS)/$(HOST_PREBUILT_TAG)/bin:$(KERNEL_TOOLCHAIN_arm):$$PATH
else
  TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/$(PREBUILT_KERNEL_TOOLS)/$(HOST_PREBUILT_TAG)/bin:$$PATH
endif
...
KERNEL_MAKE_CMD := $(BUILD_TOP)/$(PREBUILT_TOOLS)/$(HOST_PREBUILT_TAG)/bin/make
...
KERNEL_MAKE_FLAGS += LEX=$(BUILD_TOP)/$(PREBUILT_TOOLS)/$(HOST_PREBUILT_TAG)/bin/flex
KERNEL_MAKE_FLAGS += YACC=$(BUILD_TOP)/$(PREBUILT_TOOLS)/$(HOST_PREBUILT_TAG)/bin/bison
KERNEL_MAKE_FLAGS += M4=$(BUILD_TOP)/$(PREBUILT_TOOLS)/$(HOST_PREBUILT_TAG)/bin/m4
TOOLS_PATH_OVERRIDE += BISON_PKGDATADIR=$(BUILD_TOP)/$(PREBUILT_TOOLS)/common/bison
...
SOONG_CONFIG_NAMESPACES += kernelVarsPlugin

SOONG_CONFIG_kernelVarsPlugin :=

define addKernelVar
  SOONG_CONFIG_kernelVarsPlugin += $(1)
  SOONG_CONFIG_kernelVarsPlugin_$(1) := $$(subst ",\",$$($1))
endef

$(foreach v,$(KERNEL_EXPORT_TO_SOONG),$(eval $(call addKernelVar,$(v))))
